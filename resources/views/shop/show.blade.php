        ...
        <div class="max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:px-8">
            <div x-data="{ 
                    ...
                    <div class="flex sm:flex-col gap-4 sm:w-24">
                        @forelse ($product->media as $media)
                            <div @click="mainMediaUrl = '{{ asset('storage/' . $media->file_path) }}'; mainMediaType = '{{ $media->media_type }}'"
                                 :class="{ 'ring-2 ring-primary': mainMediaUrl === '{{ asset('storage/' . $media->file_path) }}' }"
                                 class="relative aspect-w-1 aspect-h-1 rounded-lg overflow-hidden cursor-pointer">
                ...
                <div class="mt-10 sm:mt-0">
                    ...
                    <p class="mt-6 text-light">{{ $product->description }}</p>

                    <form @submit.prevent="addToCart()" x-ref="cartForm" class="mt-8">
                        @csrf
                        ...
                        <div class="mt-8 flex items-center space-x-4">
                            ...
                            <div class="flex items-center border border-gray-300 rounded-md">
                                <button type="button" @click="quantity = Math.max(1, quantity - 1)" class="px-4 py-2 text-lg text-light hover:bg-gray-100 rounded-l-md">-</button>
                                <span x-text="quantity" class="w-16 text-center border-t-0 border-b-0 border-l border-r border-gray-300 py-2"></span>
                                <button type="button" @click="quantity++" class="px-4 py-2 text-lg text-light hover:bg-gray-100 rounded-r-md">+</button>
                            </div>
                            <!-- Tombol Add to Cart -->
                            <button type="submit"
